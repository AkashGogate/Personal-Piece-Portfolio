    ...
    items: [
      {
        ...
        bullets: [
          "Applied memoization, state space reduction, and dynamic programming in Python to solve multi-objective satellite observation scheduling; reduced constraint violations while preserving full mission objectives and real-time operational limits.",
          "Designed a Kafka pipeline with at-least-once delivery semantics to persist satellite priority data to MongoDB, decoupling services end-to-end and providing low-latency, fault-tolerant streaming for downstream mission systems.",
          "Deployed a containerized test suite in Docker on a Kubernetes cluster, managed by a GitHub Actions CI/CD pipeline; wrote unit tests covering every system component.",
        ],
    ...
    items: [
      {
        ...
        bullets: [
          "Built an end-to-end ML pipeline that autonomously selects and tunes scRNA-seq analysis models across scVI, scANVI, scGen, and AmortizedLDA following a hypothesis-driven approach; cut GPU resource consumption 3x via accelerated profiling and tracked all experiments with Weights & Biases.",
          "Built a TransferAgent module that automates citation mapping, literature synthesis, and claim-to-source matching for a computational biology paper-grading framework; cut peer review time by 60% across the pipeline, with GitHub Actions CI/CD managing reproducible deployment.",
        ],
      ...
      },
      {
        ...
        bullets: [
          "Built a skin cancer detection pipeline using OpenCV image preprocessing and scikit-learn classifiers (Decision Tree, KNN); reached 93% classification accuracy on dermoscopic input, cross-validated across multiple folds.",
          "Published the results as a peer-reviewed paper titled 'Early Skin Cancer Detection Improvement' through Inspirit AI's formal research program.",
        ],
    ...
    items: [
      {
        ...
        bullets: [
          "Managed end-to-end tournament logistics: scheduling, venue coordination, and player registration.",
          "Coached players of all ages and skill levels, building customized training plans and providing real-time technique feedback.",
          "Handled client communications and conflict resolution across all club events.",
        ],
      ...
      },
      {
        ...
        bullets: [
          "Founded and scaled a racket stringing business to over 50 loyal clients; delivered custom string tension and performance specs tailored to each player's style and equipment.",
          "Managed all operations end to end: client consultations, inventory, pricing, and scheduling.",
        ],
...
export const projects: Project[] = [
  {
    ...
    description: "Failure-driven agent that learns from its own mistakes. Tested on 5,000+ tasks, outperforming ReAct by 15 percentage points.",
    detail: "Failure-driven self-improvement loop: the system runs agents on tasks, captures failure cases, generates improvement instructions from those failures, and feeds them back into agent context. Outperformed the ReAct baseline by 15 percentage points across a 5,000+ task benchmark using RAG-enhanced retrieval and Transformer-based reasoning. Runs against any LLM backend without code changes.",
    tags: ["Python", "LLM APIs", "RAG", "Transformers", "AgentBench", "GitHub Actions", "CI/CD"],
  ...
  },
  {
    ...
    detail: "Built a probabilistic disease cost simulation engine that computes 5-year financial risk paths for patient profiles across a 1,080-node ICD-10 comorbidity graph encoding statistical co-occurrence between diagnoses. Integrated MEPS and CMS actuarial datasets spanning 45 million records via SQL. A RAG pipeline maps natural language symptom input to ICD-10 nodes as the retrieval layer feeding the simulation. The architecture separates NLP inference (Groq-hosted Llama) from deterministic financial logic, producing ACID-compliant, auditable cost outputs with no model hallucinations, designed to fit HIPAA-compliant, EHR-adjacent clinical decision support workflows.",
  ...
  },
  {
    ...
    description: "ML pipeline that autonomously selects and tunes scRNA-seq models across scVI, scANVI, scGen, and AmortizedLDA. Cut GPU resource use 3x.",
    detail: "Built at the Kendziorski Lab (UW-Madison). The pipeline autonomously selects and tunes analysis models for spatial transcriptomics and scRNA-seq data following a hypothesis-driven approach, cutting GPU consumption 3x through accelerated profiling. Models span scVI, scANVI, scGen, and AmortizedLDA; all experiments tracked with Weights & Biases. A companion TransferAgent module automates citation mapping and literature-to-claim matching for computational biology paper review, cutting reviewer time by 60%.",
    tags: ["Python", "LLM APIs", "Scanpy", "Seurat", "CARD", "scVI", "scANVI", "R", "Weights & Biases", "Research"],
  ...
  },
  {
    ...
    detail: "Full-stack web application with a TypeScript/React.js frontend communicating to a Node.js/FastAPI backend via a RESTful API. GPT-4o generates SAT-style practice questions from user-specified topic and difficulty parameters, with answer choices and explanations. The backend tracks answer history, validates responses against generated answer keys, and supports multi-question session generation. Hallucination filtering catches invalid model outputs before they reach the student.",
  ...
  },
  {
    ...
    description: "Real-time hand orientation tracker using OpenCV and MediaPipe. Runs at 30fps on a laptop webcam, no GPU needed.",
    detail: "A Python gesture recognition system that processes live webcam input with OpenCV for frame capture and MediaPipe for hand landmark detection and skeleton tracking. Outputs joint positions, wrist orientation, and finger positions at video frame rate. Built as a tennis coaching tool to provide quantitative grip and swing feedback; the biomechanical output also applies directly to clinical motion capture and physical therapy progress tracking.",
  ...
  },
  {
    ...
    detail: "Multi-threaded, AI-driven multi-agent simulation built in C++ with the Raylib graphics library. Autonomous bots compete for resources under configurable ecological constraints (food scarcity, predation, territory), producing emergent behaviors from simple rule sets. Designed as a gamified environmental science education tool to demonstrate carrying capacity, predator-prey dynamics, and resource competition through interactive simulation.",
  ...
  },
  {
    ...
    detail: "Native Android application in Java using the Android SDK and HttpURLConnection. Consumes USTA's GraphQL API endpoints to retrieve local tournament listings, registration deadlines, and location data, then redirects users to the USTA site for registration. Built from personal frustration with how hard the official site is to navigate on mobile.",
  ...
  },
  {
    ...
    detail: "Built a Random Forest classifier in R to predict cancer cell likelihood from micro-RNA expression sequence data. Reached 95% predictive accuracy (p < 0.05) validated via k-fold cross-validation. The feature space consists of miRNA expression levels as numeric predictors for binary cancer/non-cancer classification, trained on public genomics datasets via Bioconductor.",
